...
function ConstructPeano(lines){
	...
	lines.forEach(line => {
		...
		var newSquares = [];
		var iSquares = constructSquares(iPnt, iSeg);
		var squares = constructSquares(iSeg, fSeg);
		var fSquares = constructSquares(fSeg, fPnt);
		//newSquares.push.apply(newSquares, iSquares);
		newSquares.push.apply(newSquares, squares);
		//newSquares.push.apply(newSquares, fSquares);
		for(var s in newSquares){
			newLines.push(newSquares[s]);
		}
