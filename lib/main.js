...
function constructSquares(is, fs){
	...
	if(vertical){
		...
		var iAbove = Line(seg.i.x, seg.i.y, seg.i.x, seg.f.y + size);
		var iBelow = Line(seg.i.x, seg.i.y, seg.i.x, seg.f.y - size);
		var fAbove = Line(seg.f.x, seg.i.y, seg.f.x, seg.f.y + size);
		var fBelow = Line(seg.f.x, seg.i.y, seg.f.x, seg.f.y - size);
		squares.push(above);
		squares.push(below);
		squares.push(iAbove);
		squares.push(iBelow);
...
function ConstructPeano(lines){
	...
	lines.map(line => {
		...
		if(vertical){
			var seg0 = line.i.y + squareSize;
			var seg1 = seg0 + squareSize;
			iSeg.y = seg0;
			fSeg.y = seg1;
		}
		else{
			var seg0 = line.i.x + squareSize;
			var seg1 = seg0 + squareSize;
			iSeg.x = seg0;
			fSeg.x = seg1;
		}
